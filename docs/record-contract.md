...
Durable Records are Markdown files under `records/active/` or `records/archived/` with required YAML frontmatter.
...
Graphs, digests, reports, and context packs are generated from Durable Records into `dist/`. They are not canonical and are not loaded by default.
